...
Now, notice how the page is optimized with "download-on-demand." If a JavaScript, CSS, HTML or JSON file isn't absolutely and immediately necessary, it is not loaded until after the onLoad event fires. Many of the features that Bill Scott discusses in his [design patterns coverage of this launch](/yuiblog/2006/05/15/patterns-behind-homepage/), such as In Context Configuration and Dynamic Local Content, aren't used by each user on each visit. By downloading the code for these pieces of functionality either automatically post-load or strictly on-demand, unnecessary overhead is reduced and the page is much faster.
